...
const DEFAULT_OUT_FORMAT = 1;
const DEFAULT_JPEG_QUALITY = 100;
const DEFAULT_JPEG_SUBSAMPLING = 0;
const DEFAULT_PNG_COMPRESSION = 9;

let jpegQuality = DEFAULT_JPEG_QUALITY;
let jpegSubsampling = DEFAULT_JPEG_SUBSAMPLING;   // 0=4:4:4, 1=4:2:2, 2=4:2:0
let pngCompression = DEFAULT_PNG_COMPRESSION;

function applyDefaultFormatPreset() {
    jpegQuality = DEFAULT_JPEG_QUALITY;
    jpegSubsampling = DEFAULT_JPEG_SUBSAMPLING;
    pngCompression = DEFAULT_PNG_COMPRESSION;

    const formatSelect = document.getElementById('out-format');
    if (formatSelect) formatSelect.value = String(DEFAULT_OUT_FORMAT);
}

function renderFormatOpts() {
    const container = document.getElementById('format-opts');
    if (!container) return;
    const fmt = document.getElementById('out-format')?.value ?? String(DEFAULT_OUT_FORMAT);
    container.innerHTML = '';
...
async function loadFile(file) {
    ...
    try {
        ...
        if (BROWSER_DECODE_FORMATS.has(file.type)) {
            ...
            // Detect chroma from JPEG header; auto-fill subsampling only.
            const chroma = detectJpegChroma(origFileBytes);
            let rerenderFormatOpts = false;
            if (file.type === 'image/jpeg') {
                // Auto-fill subsampling from the detected chroma.
                if (chroma !== null) {
                    const chromaMap = { '4:4:4': 0, '4:2:2': 1, '4:2:0': 2 };
                    const nextSubsampling = chromaMap[chroma];
                    if (nextSubsampling !== undefined && nextSubsampling !== jpegSubsampling) {
                        jpegSubsampling = nextSubsampling;
                        rerenderFormatOpts = document.getElementById('out-format')?.value === '0';
                    }
                }
            }
            if (rerenderFormatOpts) renderFormatOpts();
...
function currentParams() {
    return {
        ...
        outFormat: parseInt(document.getElementById('out-format')?.value ?? String(DEFAULT_OUT_FORMAT), 10),
        jpegQuality,
        jpegSubsampling,
        pngCompression,
    };
